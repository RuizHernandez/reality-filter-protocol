...
Los skills de dominio son la capa sobre el núcleo universal: `PROTOCOL.md` gobierna la conducta, y cada skill añade los invariantes de un dominio de trabajo. Dos cubren el flujo de investigación en ingeniería bioquímica y bioprocesos del Dr. Itan Homero Ruiz-Hernández; dos cubren el trabajo en software y sistemas y en seguridad.
...
- **`computational-arch`** — la capa de software y sistemas, y el primer skill de dominio escrito para este repositorio en lugar de extraído de un flujo privado. Su regla distintiva es un **preflight de entorno** obligatorio: la cadena de herramientas se detecta, nunca se asume, generalizando las comprobaciones ad-hoc que los skills bio ya hacen. Añade además los modos de fallo propios del software: las APIs y los nombres de paquete deben verificarse contra la versión *instalada* y el registro oficial (un nombre de paquete inventado es una superficie de ataque real — *slopsquatting*); "los tests pasan" y "X es más rápido" exigen rastro ejecutable (comando, código de salida, ruta del log; n corridas con dispersión) o son `[U]`/`[S]`; las afirmaciones sobre concurrencia se topan en `[I]`, porque el código con condición de carrera pasa la mayoría de las corridas; y las operaciones destructivas muestran su alcance antes de ejecutarse. También deja explícito que `[E]` significa *verificado empíricamente*, nunca *seguro*: un análisis estático limpio es evidencia sobre la herramienta, no sobre el código.
- **`cybersecurity`** — trabajo de seguridad, **defensivo y autorizado únicamente**. Invierte deliberadamente la estructura de `bio-ruiz-hernandez`: allí la capa ética es §5, aquí es **§1**, porque en investigación la ética gobierna cómo se reporta un resultado, mientras que en seguridad gobierna si la acción puede ejecutarse siquiera. §1 exige establecer autorización y alcance antes que nada, trata el alcance como límite duro que nunca se amplía por iniciativa propia, obliga al aislamiento en laboratorio para código no confiable, y cubre divulgación responsable y datos de terceros. El resto aplica el filtro de realidad a los modos de fallo propios de la seguridad: los identificadores CVE/CWE se verifican contra bases autoritativas antes de citarse (los modelos los inventan); un hallazgo sin reproducción documentada se queda en `[S]`, y "lo reportó la herramienta" es `[I]`, porque los falsos positivos son el caso normal; la severidad se argumenta en el contexto de *este* despliegue en lugar de heredarse del aviso; y "no se encontraron vulnerabilidades" es un enunciado sobre la cobertura, nunca sobre el sistema.
...
Por la misma razón existen los dos skills de dominio. El protocolo núcleo gobierna la conducta general del agente, pero no responde las preguntas que aparecen al llevarlo a la investigación real: ¿quién firma la interpretación de un resultado?, ¿cómo se evita que un modelo reporte como válidas métricas infladas por fuga de datos?, ¿qué formato exige una revista? **`bio-ruiz-hernandez`** importa porque convierte la honestidad genérica en *integridad académica operativa*: sin su capa ética, un agente perfectamente "honesto" puede aun así redactar conclusiones simulando autoría, aceptar datos sensibles sin anonimizar o entregar cifras sin código que las respalde. **`numerical-data-analysis`** importa porque el error más costoso en ML aplicado no es un modelo malo, sino un modelo bueno *medido mal*: la fuga de datos infla métricas silenciosamente, y ninguna etiqueta de veracidad la detecta por sí sola — hacen falta reglas estructurales (`fit` solo en Train, pipelines obligatorios) que la hagan imposible por construcción.
